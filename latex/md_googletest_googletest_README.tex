\hypertarget{md_googletest_googletest_README_autotoc_md270}{}\doxysubsubsection{Setup}\label{md_googletest_googletest_README_autotoc_md270}
To build Google\+Test and your tests that use it, you need to tell your build system where to find its headers and source files. The exact way to do it depends on which build system you use, and is usually straightforward.\hypertarget{md_googletest_googletest_README_autotoc_md271}{}\doxysubsection{Build with CMake}\label{md_googletest_googletest_README_autotoc_md271}
Google\+Test comes with a CMake build script (\href{https://github.com/google/googletest/blob/master/CMakeLists.txt}{\texttt{ CMake\+Lists.\+txt}}) that can be used on a wide range of platforms (\char`\"{}\+C\char`\"{} stands for cross-\/platform.). If you don\textquotesingle{}t have CMake installed already, you can download it for free from \href{http://www.cmake.org/}{\texttt{ http\+://www.\+cmake.\+org/}}.

CMake works by generating native makefiles or build projects that can be used in the compiler environment of your choice. You can either build Google\+Test as a standalone project or it can be incorporated into an existing CMake build for another project.\hypertarget{md_googletest_googletest_README_autotoc_md272}{}\doxysubsubsection{Standalone CMake Project}\label{md_googletest_googletest_README_autotoc_md272}
...
On Mac OS X with Xcode installed, a {\ttfamily .xcodeproj} file will be generated.\hypertarget{md_googletest_googletest_README_autotoc_md273}{}\doxysubsubsection{Incorporating Into An Existing CMake Project}\label{md_googletest_googletest_README_autotoc_md273}
...
\begin{DoxyItemize}
\item Import Google\+Test by using {\ttfamily find\+\_\+package} (or {\ttfamily pkg\+\_\+check\+\_\+modules}). For example, if {\ttfamily find\+\_\+package(\+GTest CONFIG REQUIRED)} is succeed, you can use the libraries as {\ttfamily GTest\+::gtest}, {\ttfamily GTest\+::gmock}.
\end{DoxyItemize}
...
\item Use CMake to download Google\+Test as part of the build\textquotesingle{}s configure step. This is just a little more complex, but doesn\textquotesingle{}t have the limitations of the other methods.
\end{DoxyItemize}

The last of the above methods is implemented with a small piece of CMake code in a separate file (e.\+g. {\ttfamily CMake\+Lists.\+txt.\+in}) which is copied to the build area and then invoked as a sub-\/build {\itshape during the CMake stage}. That directory is then pulled into the main build with {\ttfamily add\+\_\+subdirectory()}. For example\+:

New file {\ttfamily CMake\+Lists.\+txt.\+in}\+:
...
Existing build\textquotesingle{}s {\ttfamily CMake\+Lists.\+txt}\+:
...
Note that this approach requires CMake 2.\+8.\+2 or later due to its use of the {\ttfamily External\+Project\+\_\+\+Add()} command. The above technique is discussed in more detail in \href{http://crascit.com/2015/07/25/cmake-gtest/}{\texttt{ this separate article}} which also contains a link to a fully generalized implementation of the technique.\hypertarget{md_googletest_googletest_README_autotoc_md274}{}\doxyparagraph{Visual Studio Dynamic vs Static Runtimes}\label{md_googletest_googletest_README_autotoc_md274}
...
Enabling this option will make gtest link the runtimes dynamically too, and match the project in which it is included.\hypertarget{md_googletest_googletest_README_autotoc_md275}{}\doxysubsubsection{C++ Standard Version}\label{md_googletest_googletest_README_autotoc_md275}
An environment that supports C++11 is required in order to successfully build Google\+Test. One way to ensure this is to specify the standard in the top-\/level project, for example by using the {\ttfamily set(\+CMAKE\+\_\+\+CXX\+\_\+\+STANDARD 11)} command. If this is not feasible, for example in a C project using Google\+Test for validation, then it can be specified by adding it to the options for cmake via the {\ttfamily DCMAKE\+\_\+\+CXX\+\_\+\+FLAGS} option.\hypertarget{md_googletest_googletest_README_autotoc_md276}{}\doxysubsection{Tweaking Google\+Test}\label{md_googletest_googletest_README_autotoc_md276}
...
We list the most frequently used macros below. For a complete list, see file \href{https://github.com/google/googletest/blob/master/googletest/include/gtest/internal/gtest-port.h}{\texttt{ include/gtest/internal/gtest-\/port.\+h}}.\hypertarget{md_googletest_googletest_README_autotoc_md277}{}\doxysubsection{Multi-\/threaded Tests}\label{md_googletest_googletest_README_autotoc_md277}
