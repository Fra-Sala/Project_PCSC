\label{GMockFaq_md_googletest_googlemock_docs_gmock_faq}%
\Hypertarget{GMockFaq_md_googletest_googlemock_docs_gmock_faq}%
\hypertarget{GMockFaq_autotoc_md111}{}\doxysubsection{When I call a method on my mock object, the method for the real object is invoked instead. What\textquotesingle{}s the problem?}\label{GMockFaq_autotoc_md111}
In order for a method to be mocked, it must be {\itshape virtual}, unless you use the \href{cook_book.md\#MockingNonVirtualMethods}{\texttt{ high-\/perf dependency injection technique}}.\hypertarget{GMockFaq_autotoc_md112}{}\doxysubsection{Can I mock a variadic function?}\label{GMockFaq_autotoc_md112}
...
\end{DoxyCode}
\hypertarget{GMockFaq_autotoc_md114}{}\doxysubsection{I can\textquotesingle{}t figure out why g\+Mock thinks my expectations are not satisfied. What should I do?}\label{GMockFaq_autotoc_md114}
...
If you see the message \char`\"{}\+The mock function has no default action set, and its return type has no default value set.\char`\"{}, then try \href{for_dummies.md\#DefaultValue}{\texttt{ adding a default action}}. Due to a known issue, unexpected calls on mocks without default actions don\textquotesingle{}t print out a detailed comparison between the actual arguments and the expected arguments.\hypertarget{GMockFaq_autotoc_md115}{}\doxysubsection{My program crashed and $<$tt$>$\+Scoped\+Mock\+Log$<$/tt$>$ spit out tons of messages. Is it a g\+Mock bug?}\label{GMockFaq_autotoc_md115}
...
\end{DoxyCode}
\hypertarget{GMockFaq_autotoc_md116}{}\doxysubsection{How can I assert that a function is NEVER called?}\label{GMockFaq_autotoc_md116}
...
\end{DoxyCode}
\hypertarget{GMockFaq_autotoc_md117}{}\doxysubsection{I have a failed test where g\+Mock tells me TWICE that a particular expectation is not satisfied. Isn\textquotesingle{}t this redundant?}\label{GMockFaq_autotoc_md117}
...
By changing {\ttfamily $\sim$\+Base()} to virtual, {\ttfamily $\sim$\+Derived()} will be correctly called when {\ttfamily delete p} is executed, and the heap checker will be happy.\hypertarget{GMockFaq_autotoc_md119}{}\doxysubsection{The \char`\"{}newer expectations override older ones\char`\"{} rule makes writing expectations awkward. Why does g\+Mock do that?}\label{GMockFaq_autotoc_md119}
...
Also, you can control the verbosity by specifying {\ttfamily -\/-\/gmock\+\_\+verbose=error}. Other values are {\ttfamily info} and {\ttfamily warning}. If you find the output too noisy when debugging, just choose a less verbose level.\hypertarget{GMockFaq_autotoc_md121}{}\doxysubsection{How can I delete the mock function\textquotesingle{}s argument in an action?}\label{GMockFaq_autotoc_md121}
...
\end{DoxyCode}
\hypertarget{GMockFaq_autotoc_md122}{}\doxysubsection{How can I perform an arbitrary action on a mock function\textquotesingle{}s argument?}\label{GMockFaq_autotoc_md122}
...
\end{DoxyCode}
\hypertarget{GMockFaq_autotoc_md123}{}\doxysubsection{My code calls a static/global function. Can I mock it?}\label{GMockFaq_autotoc_md123}
...
This Google Testing Blog \href{https://testing.googleblog.com/2008/06/defeat-static-cling.html}{\texttt{ post}} says it excellently. Check it out.\hypertarget{GMockFaq_autotoc_md124}{}\doxysubsection{My mock object needs to do complex stuff. It\textquotesingle{}s a lot of pain to specify the actions. g\+Mock sucks!}\label{GMockFaq_autotoc_md124}
...
See this \href{cook_book.md\#mocking-side-effects}{\texttt{ recipe}} for more details and an example.\hypertarget{GMockFaq_autotoc_md128}{}\doxysubsection{I have a huge mock class, and Microsoft Visual C++ runs out of memory when compiling it. What can I do?}\label{GMockFaq_autotoc_md128}
